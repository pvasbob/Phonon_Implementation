...
program pnfam
   ...
   call write_output_file_header(.true.)
   
   
   select case(fam_mode)
      case("STR")
         
         call init_str_mode(.true.)
         ...
         do i=1, nr_points
            ...
            call pnfam_solve(omega, half_width, Ep, En, vp, up, vn, un, max_iter, &
               convergence_epsilon, quench_residual_int, f%beta_minus, str(:), iter, cstr(:))
            call cpu_time(t_finish)
            
            ! Log and save the result
            call log_str_point(.true.)
            call write_output_str_point(.true.)
