...
!$ use omp_lib, only : omp_get_max_threads, omp_get_wtime
   ...
   use hfbtho_basis,        only : ft_active, ft_temp, ft_fn, ft_fp, hfb_npr, pwi_qp_p, &
                                   pwi_qp_n, hfb_blo_active, get_HFBTHO_solution,       &
                                   get_hfbtho_betadecay_properties
   use interaction,         only : init_skyrme, interaction_name, print_interaction_params,&
                                   skip_residual_interaction
   ...
   use phasespace,          only : get_ratint_coeffs, ratint_eval_psi
   use logger
   implicit none
   ...
   real(dp) :: energy_shift_prot, energy_shift_neut
   real(dp) :: quench_residual_int
   ...
   !--------------------------------------------------QQst      modify30
   complex(dp), dimension(:), allocatable, public :: cstr_mini
   !--------------------------------------------------QQend     modify30
...
contains
   ...
   subroutine init_common ! except the interaction parameters
      ...
      ! Q.P. energy shifts
      Ep(:) = Ep(:) + energy_shift_prot
      En(:) = En(:) + energy_shift_neut
      ...
      str(:) = 0; cstr(:) = 0
      !----------------------------QQst         modify30
      allocate(cstr_mini(1:nxterms+1))
      cstr_mini(:) = 0
      !----------------------------QQend        modify30
      ...
      if (skip_residual_interaction) quench_residual_int = 0.0
      close(fnml)
   ...
   subroutine log_header
      ...
      call writelog("")
      write(st,'(1A,1F6.3," MeV")') "Energy shift (protons): ", energy_shift_prot; call writelog(st)
      write(st,'(1A,1F6.3," MeV")') "Energy shift (neutrons):", energy_shift_neut; call writelog(st)
      write(st,'(1A,1F6.3," (x dH")') "Interaction Quenching:", quench_residual_int; call writelog(st)
      ...
      select case(fam_mode)
         ...
         case("STR")
            write(st,'(A,F8.4,A)') "Half-width gamma:    ", half_width, " MeV" ; call writelog(st)
         ...
         case("CONTOUR")
            ...
            write(st,'(2A)')      "Use Gauss-Legendre contour:        ", merge("Yes","No ",use_gauleg_contour); call writelog(st)
            write(st,'(A,1I0)')   "Rational-fct interp. Points:       ", ratint_points ; call writelog(st)
            write(st,'(A,1I0)')   "PSI Quadrature Points              ", ratint_psi_gl_points ; call writelog(st)
            ...
            write(st,'(A,1F7.3," MeV")') "Contour left anchor point:    ", contour_anchor; call writelog(st)
            if (contour_anchor < 0) then
               write(st,'(A)') '' ; call writelog(st)
               write(st,'(A)') repeat('*',80) ; call writelog(st)
               write(st,'(A)') txtc('NOTICE!!!',80)
               call writelog(st)
               write(st,'(A)') txtc('PNFAM has been asked to put the left contour anchor below E=0.',80)
               call writelog(st)
               write(st,'(A)') txtc('In previous versions, setting contour_anchor < 0 simply', 80)
               call writelog(st)
               write(st,'(A)') txtc('assigned the default value automatically. This is no longer the case.', 80)
               call writelog(st)
               write(st,'(A)') txtc('Most normal operations should have contour_anchor = 0.', 80)
               call writelog(st)
               write(st,'(A)') repeat('*',80) ; call writelog(st)
            end if
      ...
      call writelog("")
      write(st,'(2A)') 'Odd-nucleus mode: ', trim(merge('On ', 'Off', hfb_blo_active))
      call writelog(st)
      
      ! Odd nucleus
      if (hfb_blo_active) call log_odd_nucleus_header
   ...
   subroutine log_str_point(use_cstr)
      implicit none
      character(len=160) :: st
      logical, optional, intent(in) :: use_cstr
      logical :: comp
      
      comp = .false.
      if (present(use_cstr)) then
          if (use_cstr .eqv. .true.) then
              comp = .true.
          end if
      end if
      ...
      if (comp) then
         ...
         write(st,'(1F11.6,11X,1F16.6,9x,1F16.6,4X,1I10,4X,1F8.2,4X,1F12.2)') omega, cstr(1), &
   ...
   subroutine init_str_mode(use_cstr)
      implicit none
      character(len=160) :: st
      logical, optional, intent(in) :: use_cstr
      logical :: comp
      ...
      call writelog("")
      if (comp) then
         call writelog("Energy [MeV]     Re(Strength) [MeV^-1]    Im(Strength) [MeV^-1]    &
            &Iterations     Time (s)     T/Iter. (s)")
         call writelog("-------------------------------------------------------------------&
            &---------------------------------------")
      else
   ...
   ! ASCII output file
   !---------------------------------------------------------------------------
   subroutine write_output_file_header(use_cstr)
      implicit none
      logical, optional, intent(in) :: use_cstr
      logical :: comp
      comp = .false.
      ...
      case("STR")
         ...
         if (comp) then
            write(fno,'(4A)',advance='no') "#", txtr("Re(EQRPA)", 29), &
      ...
      case("CONTOUR")
         ...
         write(fno,'(2A)', advance='no') txtr("Re(f(E)_rat)", 34), txtr("Im(f(E)_rat)", 34)
         write(fno,'(3A)') txtr("Re(dz/dt)", 30), txtr("Im(dz/dt)", 30), txtr("GL_Weights",30)
         
      end select
   end subroutine
   
   
   subroutine write_output_str_point(use_cstr)
      implicit none
      logical, optional, intent(in) :: use_cstr
      logical :: comp
      comp = .false.
      ...
      if (comp) then
         write(fno,'(1F30.19)',advance='no') omega
         do istr=1, size(cstr)
            write(fno,'(2ES34.19)',advance='no') cstr(istr)
         end do
      else
   ...
   !----------------------------------------------------------------------------
   ! Log some odd-nucleus diagnostics
   !----------------------------------------------------------------------------
   subroutine log_odd_nucleus_header
      
      use constants,    only : IT_NEUTRON, IT_PROTON, strpar
      use hfbtho_basis, only : hfb_blo_qp, npar, nl, ns, iqp_to_block_and_state
      ...
      write(st,'(a)') repeat('-',33)
      call writelog(st)
      
      if (hfb_blo_qp(IT_NEUTRON) /= 0) then
         write(st,'("Neutron blocking ... : On")')
         call writelog(st)
         
         write(st,'("Blocked level ...... : ", 1i0, "/2", a)') &
            2*nl(hfb_blo_qp(IT_NEUTRON)) + ns(hfb_blo_qp(IT_NEUTRON)), &
            strpar(npar(hfb_blo_qp(IT_NEUTRON)))
         call writelog(st)
            
         write(st,'("QP energy .......... : ", 1f8.4, " MeV")') En(hfb_blo_qp(IT_NEUTRON))
         call writelog(st)
            
         call iqp_to_block_and_state(hfb_blo_qp(IT_NEUTRON), ib, is)
         write(st,'("HFBTHO index ....... : ", 1i0, " (block = ", 1i0, ", state = ", 1i0, ")")') &
            hfb_blo_qp(IT_NEUTRON), ib, is
         call writelog(st)
      else
         write(st,'("Neutron blocking ... : Off")')
         call writelog(st)
      end if
      
      if (hfb_blo_qp(IT_PROTON) /= 0) then
         write(st,'("Proton blocking .... : On")')
         call writelog(st)
         
         write(st,'("Blocked level ...... : ", 1i0, "/2", a)') &
            2*nl(hfb_blo_qp(IT_PROTON)) + ns(hfb_blo_qp(IT_PROTON)), &
            strpar(npar(hfb_blo_qp(IT_PROTON)))
         call writelog(st)
            
         write(st,'("QP energy .......... : ", 1f8.4, " MeV")') Ep(hfb_blo_qp(IT_PROTON))
         call writelog(st)
            
         call iqp_to_block_and_state(hfb_blo_qp(IT_PROTON), ib, is)
         write(st,'("HFBTHO index ....... : ", 1i0, " (block = ", 1i0, ", state = ", 1i0, ")")') &
            hfb_blo_qp(IT_PROTON), ib, is
         call writelog(st)
      else
         write(st,'("Proton blocking .... : Off")')
         call writelog(st)
      end if
      
   end subroutine log_odd_nucleus_header
