...
contains
   ...
   subroutine init_fam_mapping(op)
      ...
      !--------------------------------
      if(allocated(Ph_select)) deallocate(Ph_select)
      Allocate(Ph_select(nb, nb))
      Ph_select=0
      !
      NumberofHpn = 0
      ...
      !
      if(allocated(Ph_sum_db_colm1)) deallocate(Ph_sum_db_colm1)
      allocate(Ph_sum_db_colm1(nb))
      Ph_sum_db=0
      Ph_sum_db_colm1=0
      do qqj=1,nb
         Ph_sum_db=Ph_sum_db + db(qqj)
         Ph_sum_db_colm1(qqj)=Ph_sum_db - db(qqj)
      ...
      !
      open(100,file='Ph_binary_output_extfield_GT0',status='unknown',form='unformatted')
      write(100) nb
      write(100) NumberofHpn
      write(100) db
      write(100) Ph_sum_db
      write(100) Ph_sum_db_colm1
      write(100) Ph_select
      close(100)
      !
      !--------------------------------------------
      ! check if select matrix gives correct number of element
      !--------------------------------------------
      NumberofHpn_check=0
      do qqj=1,nb
         do qqi=1,nb
