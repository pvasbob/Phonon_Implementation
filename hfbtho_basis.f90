...
!   - get_HFBTHO_occupations
!   - hfb_density
!   - get_toymodel
!   - iqp_to_block_and_state
!
! Functions:
!   - lowest_energy_qp
!   - HFBTHO_map_int_to_logical
!   - qp_t_reverse
!------------------------------------------------------------------------------
...
module hfbtho_basis
   ...
   integer :: nghl  ! number of 2D quadrature points in coordinate space (n-gausshermite*n-gausslageurre)
   integer :: dqp   ! dimension of the basis
   ...
   real(dp), allocatable, target :: wf(:,:), wfdr(:,:), wfdz(:,:), wfd2(:,:), wfdp(:,:), wfd2_all(:,:)
   ...
   integer :: hfb_nbx = -1         ! the number of blocks 
   !integer :: nb                  ! the number of blocks (declared in blockmatrix_type)
   !integer, allocatable :: db(:)  ! the number of states (declared in blockmatrix_type)
   ...
   ! The following array gives the number of basis states with spin up in each block
   integer, allocatable :: num_spin_up(:)
   ...
   ! Blocking/odd-mass quantities (v. 10)
   logical :: hfb_blo_active = .false.
   integer :: hfb_blo_qp(2)  = 0
   integer :: keyblo(2)

contains

   ! Auxiliary subroutine to reorder basis states according to ns
   subroutine reorder_blockmatrix_row(U, new_order)
      ...
      do iblock=1, nb
         do ic=1, db(iblock)
            U%elem(im+1:im+db(iblock)) = U%elem(im-ir+new_order(ir+1:ir+db(iblock)))
            im = im + db(iblock)
         end do
         ir = ir + db(iblock)
      end do
   end subroutine reorder_blockmatrix_row
   ...
   subroutine get_HFBTHO_solution(fn, Ep, Vp, Up, En, Vn, Un)
      ...
      integer  :: dmat        ! number of non-trivial matrix elements in U, V, etc.
      ...
      integer  :: blo_qp, blo_blo, blo_level, blo_blo_chk, blo_level_chk ! blocking
      real(dp) :: blo_en                                                         ! blocking
      ...
      ! Fix a small particle-number bug with blocking in HFBTHO
      if (hfb_npr(1)+hfb_npr(2) /= hfb_npr(3)) then
         write(*,'("Fixing an HFBTHO particle number bug: A was ",1i0,", is now ",1i0)') &
            hfb_npr(3), hfb_npr(1)+hfb_npr(2)
         hfb_npr(3) = hfb_npr(1)+hfb_npr(2)
      end if
      ...
      nb = 2*nb
      allocate(db(nb))
      read(f) db(1:nb/2)       ! number of basis states in each block (id(ib) in hfbtho)
      db(nb/2+1:) = db(1:nb/2) ! db = [id, id] - twice as many for explicit time reversed states
      dqp = sum(db)            ! number of basis states (or, equivalently, qp states - if no pwi)
      dmat = sum(db*db)        ! number of nontrivial matrix elements in U, V (db*db = [id^2, id^2])
      ...
      allocate(new_order(dqp))
      allocate(num_spin_up(nb))
      new_order = 0; num_spin_up = 0
      im = 0
      do iblock=1, nb
         iqp = 0
         do ic=im+1, im+db(iblock)
            if (ns(ic)>0) then
               iqp = iqp + 1
               new_order(im+iqp) = ic
            end if
         end do
         num_spin_up(iblock) = iqp
         do ic=im+1, im+db(iblock)
            if (ns(ic)<0) then
               iqp = iqp + 1
               new_order(im+iqp) = ic
      ...
      wfdz(:,:) = wfdz(:,new_order)
      wfd2(:,:) = wfd2(:,new_order)
      call reorder_blockmatrix_row(Un, new_order)
      call reorder_blockmatrix_row(Vn, new_order)
      call reorder_blockmatrix_row(Up, new_order)
      call reorder_blockmatrix_row(Vp, new_order)
      deallocate(new_order)
#endif
#endif

      ! pre-compute some quantities for module hamiltonian
      do i=1, dqp
         ! derivative of phi, without imaginary unit "i" at the beginning
         wfdp(:,i) = y(:)*nl(i)*wf(:,i)

         ! laplacian of wavefunction with phi-derivative added
      ...
      if (version >= 10) then
         ! Neutrons - in hfbtho: keyblo(1), KqpN(blok1k2d(1)), ib, is, REqpN(KqpN(blok1k2d(1))) 
         ! Recall keyblo is the index of a given blocking candidate
         read(f) keyblo(1), blo_qp, blo_blo, blo_level, blo_en
         
         if (keyblo(1) .Ne. 0) then
            hfb_blo_active = .true.
            hfb_blo_qp(IT_NEUTRON) = blo_qp
            
            ! Check that we have identified the correct state
            if (abs(blo_en-En(blo_qp)) > 1d-10) then
               write(*,'(a,1f6.4,a,1f6.4,a)') "ERROR: mismatch in blocked q.p. energies (", &
                  blo_en, "vs. ", En(blo_qp), ")."
               stop
            end if
            
            call iqp_to_block_and_state(blo_qp, blo_blo_chk, blo_level_chk)
            
            if (blo_blo_chk /= blo_blo .or. blo_level_chk /= blo_level) then
               write(*,'(a,"(", 1i0, ",", 1i0, ") vs. (", 1i0, ",", 1i0, ")")') &
                  'ERROR: mismatch in blocked q.p. block and state: ', blo_blo, blo_level, &
                  blo_blo_chk, blo_level_chk
               stop
            end if
         end if
         
         ! Protons - in hfbtho: keyblo(2), KqpN(blok1k2d(2)), ib, is, REqpN(KqpN(blok1k2d(2)))  
         read(f) keyblo(2), blo_qp, blo_blo, blo_level, blo_en
         
         if (keyblo(2) .Ne. 0) then
            hfb_blo_active = .true.
            hfb_blo_qp(IT_PROTON) = blo_qp
            
            ! Check that we have identified the correct state
            if (abs(blo_en-Ep(blo_qp)) > 1d-10) then
               write(*,'(a,1f6.4,a,1f6.4,a)') "ERROR: mismatch in blocked q.p. energies (", &
                  blo_en, "vs. ", Ep(blo_qp), ")."
               stop
            end if
   ...
   subroutine iqp_to_block_and_state(iqp, iblock, istate)
      ...
      istate = 0
      do iblock=1, nb
         if ((istate+db(iblock)) > iqp) then
            exit
         else
            istate = istate+db(iblock)
         end if
      end do
      istate = iqp-sum(db(1:iblock-1))
