...
! calculating mean fields and using BLAS operations, trading off clarity of the code.
!
...
contains
   ...
   subroutine add_field(a, symbol, add)
      ...
      a%re = a%re + symbol*add%re; a%im = a%im + symbol*add%im
   ...
   subroutine meanfield(ds, reh, imh)
      ...
      !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(ix,iy,iz,wf_b_all)
      do ib = 1, dqp
         ! Column vector |b>
         wf_b_all(0)%p => wf(:, ib)
   ...
   subroutine pairingfield(ds, red, imd)
      ...
      do ib = 1, dqp
         if (ns(ib)==1) then
            ...
            stop 'Inconsistent spin in helpers.pairingfield'
         end if
      end do
